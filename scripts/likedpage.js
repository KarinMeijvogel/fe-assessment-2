...
const matches = JSON.parse(localStorage.data).filter(
  person => person.liked === true && person.likedMe === true
);
...
function dislike(e) {
  ...
  const clickedUser = data.find(
    person => person.firstName === e.target.closest('figure').dataset.id
  );
...
function newMatch() {
  // update the data
  const newMatch = data.find(
    person => person.liked === true && person.likedMe === null
  );
